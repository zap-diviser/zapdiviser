...
// @ts-ignore
const baseUrl = import.meta.env.VITE_BACKEND_URL || 'https://backend.zapdiviser.com';

export type ErrorWrapper<TError> = TError | { status: 'unknown'; payload: string };

export type ZapdiviserFetcherOptions<TBody, THeaders, TQueryParams, TPathParams> = {
  url: string;
  method: string;
  body?: TBody;
  headers?: THeaders;
...
export async function zapdiviserFetch<
  TData,
  TError,
...
>({
  ...
  queryParams,
  signal
}: ZapdiviserFetcherOptions<TBody, THeaders, TQueryParams, TPathParams>): Promise<TData> {
  try {
